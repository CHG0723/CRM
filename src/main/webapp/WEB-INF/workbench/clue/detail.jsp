...
<body>
		...
		<div class="modal-dialog" role="document" style="width: 80%;">
			<div class="modal-content">
				...
				<div class="modal-footer">
					...
					<button type="button" onclick="saveBind()" class="btn btn-primary">关联</button>
				</div>
			</div>
		</div>
	</div>
	...
	<div id="father" style="position: relative; top: 40px; left: 40px;">
		...
		<div class="remarkDiv" id="deleteRemarkDiv${clueRemark.id}" style="height: 60px;">
			...
			<div style="position: relative; top: -40px; left: 40px;" >
				...
				<font color="gray">线索</font> <font color="gray">-</font> <b>${clue.fullname}-${clue.company}</b> <small style="color: gray;"> ${clue.createTime} 由${clue.createBy}创建</small>
				<div style="position: relative; left: 500px; top: -30px; height: 30px; width: 100px; display: none;">
					...
					&nbsp;&nbsp;&nbsp;&nbsp;
					<a class="myHref" id="deleteClueRemark${clueRemark.id}" href="javascript:void(0);"><span class="glyphicon glyphicon-remove" style="font-size: 20px; color: #E6E6E6;"></span></a>
				</div>
			</div>
		</div>
			...
			<script>
				//点击删除按钮,删除备注
				$('#deleteClueRemark${clueRemark.id}').click(function () {
					//把隐藏域的内容改成当前备注的主键
					...
					$.ajax({
						...
						success : function(data){
							if (data.success){
								alert(data.mess);
								//删除页面元素
								$('#deleteRemarkDiv${clueRemark.id}').remove();
							}
				...
				<table class="table table-hover" style="width: 900px;">
					<thead>
						<tr style="color: #B3B3B3;">
							...
							<td>结束日期</td>
							<td>所有者</td>
							<td><button class="btn btn-danger" onclick="deleteManyBind()">解除多个绑定</button></td>
						</tr>
					</thead>
...
//点击解除绑定按钮，解除线索和市场活动关系
	...
	//关联市场活动模态窗口,在输入框中输入内容后,触发键盘事件
	//对于模态窗口,如果按下回车键,模态窗口会默认刷新当前页面
	$('#activityName').keypress(function () {
		if (event.keyCode == 13){
		    //按下回车键,发送请求
            $.ajax({
